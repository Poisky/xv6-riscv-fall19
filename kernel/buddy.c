...
//将index位置上的alloc取反
void alloc_toggle(char *array,int index){
  index = index / 2;
  char b = array[index/8];
  char m = (1 << (index % 8));
  array[index/8] = (b ^ m);
...
//判断alloc标志是否为1
int alloc_get(char *array,int index){
  index = index / 2;
  char b = array[index/8];
  char m = (1 << (index % 8));
  return (b & m) == m;
...
bd_malloc(uint64 nbytes)
{
  ...
  alloc_toggle(bd_sizes[k].alloc, blk_index(k, p));
  for(; k > fk; k--) {
    // split a block at size k and mark one half allocated at size k-1
    // and put the buddy on the free list at size k-1
    char *q = p + BLK_SIZE(k-1);   // p's buddy
    bit_set(bd_sizes[k].split, blk_index(k, p));
    alloc_toggle(bd_sizes[k-1].alloc, blk_index(k-1, p));
...
bd_free(void *p) {
  ...
  for (k = size(p); k < MAXSIZE; k++) {
    ...
    alloc_toggle(bd_sizes[k].alloc, bi);  // free p at size k
    if (alloc_get(bd_sizes[k].alloc, buddy)) {  // is buddy allocated?
...
bd_mark(void *start, void *stop)
{
  ...
  for (int k = 0; k < nsizes; k++) {
    ...
    for(; bi < bj; bi++) {
      ...
      alloc_toggle(bd_sizes[k].alloc,bi);
    }
  }
}
...
int in_range(void *bd_left,void *bd_right,void *addr){
  return (addr >= bd_left)&&(addr < bd_right);
}
...
bd_initfree_pair(int k, int bi, void *bd_left, void *bd_right) {
  int buddy = (bi % 2 == 0) ? bi+1 : bi-1;
  int free = 0;
  if(alloc_get(bd_sizes[k].alloc, bi)) {
    // one of the pair is free
    free = BLK_SIZE(k);
    if(in_range(bd_left, bd_right, addr(k, buddy)))
